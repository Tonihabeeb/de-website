  ...
  try {
    ...
    for (const file of backupFiles) {
      if (file.endsWith('.db') || file.endsWith('.sql')) {
        ...
        const match = file.match(/backup-(\d{4}-\d{2}-\d{2})-(\d{2}-\d{2}-\d{2})-([a-z]+)-(.+)\.(db|sql)/);
        ...
        if (match) {
          ...
          const backup: BackupInfo = {
            ...
            tables: ['users', 'pages', 'projects', 'media', 'site_settings', 'navigation_menus', 'content_versions'],
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { 
      ...
      encryption = false,
      retentionDays = 30,
      includeTables = ['users', 'pages', 'projects', 'media', 'site_settings', 'navigation_menus', 'content_versions']
    } = body;
...
async function cleanupOldBackups(backupsDir: string, retentionDays: number): Promise<number> {
