  ...
  try {
    ...
    switch (format) {
      case 'json':
        ...
        response.headers.set('Content-Disposition', `attachment; filename="report-${id}.json"`);
        break;
      ...
      case 'csv':
        ...
        response.headers.set('Content-Disposition', `attachment; filename="report-${id}.csv"`);
        break;
      ...
      case 'excel':
        ...
        response.headers.set('Content-Disposition', `attachment; filename="report-${id}.xlsx"`);
        break;
      ...
      case 'pdf':
        ...
        response.headers.set('Content-Disposition', `attachment; filename="report-${id}.pdf"`);
        break;
...
async function generateContentReport(filters: any) {
  ...
  return {
    ...
    summary: {
      ...
      published_pages: contentData.filter((p: any) => p.status === 'published').length,
      draft_pages: contentData.filter((p: any) => p.status === 'draft').length,
      total_views: contentData.reduce((sum: number, p: any) => sum + p.views, 0)
    },
    data: contentData
  };
}
...
async function generateUserReport(filters: any) {
  ...
  return {
    ...
    summary: {
      ...
      total_pages_created: userData.reduce((sum: number, u: any) => sum + u.pages_created, 0)
    },
    data: userData
  };
}
...
async function generateSystemReport(filters: any) {
  ...
  return {
    ...
    filters,
    summary: {
      database_size_mb: Math.round(systemData.database_size / 1024 / 1024 * 100) / 100,
      total_records: systemData.total_pages + systemData.total_projects + systemData.total_users,
      system_uptime_hours: Math.round(systemData.system_uptime / 3600 * 100) / 100
    },
    data: systemData
  };
}
