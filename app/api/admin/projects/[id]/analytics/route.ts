...
export async function GET(
  ...
  try {
    ...
    `);
    const timelineEvents = timelineStmt.all(id, startDate.toISOString()) as any[];

    // Get project media count
    const mediaStmt = db.prepare('SELECT COUNT(*) as count FROM media WHERE project_id = ?');
    ...
    const totalMilestones = timelineEvents.filter(e => e.type === 'milestone').length;
    const completedMilestones = timelineEvents.filter(e => e.type === 'milestone' && e.status === 'completed').length;
    const progressPercentage = totalMilestones > 0 ? (completedMilestones / totalMilestones) * 100 : 0;

    // Get status distribution
    const statusStmt = db.prepare('SELECT status, COUNT(*) as count FROM projects GROUP BY status');
    const statusDistribution = statusStmt.all() as any[];

    // Calculate project duration
    const projectStart = new Date(project.created_at);
    const projectDuration = Math.ceil((now.getTime() - projectStart.getTime()) / (1000 * 60 * 60 * 24));
    ...
    const analytics = {
      ...
      timeline: {
        ...
        events_by_type: timelineEvents.reduce((acc, event) => {
      ...
      performance: {
        on_schedule: project.status === 'in-progress' || project.status === 'completed',
