...
export default function EditProject() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          team_members: projectData.team_members ? projectData.team_members.split(',').filter(Boolean) : [],
          tags: projectData.tags ? projectData.tags.split(',').filter(Boolean) : [],
  ...
  const addTeamMember = () => {
    if (newTeamMember.trim() && !formData.team_members.includes(newTeamMember.trim())) {
      handleInputChange('team_members', [...formData.team_members, newTeamMember.trim()]);
      setNewTeamMember('');
    }
  };

  const removeTeamMember = (memberToRemove: string) => {
    handleInputChange('team_members', formData.team_members.filter(member => member !== memberToRemove));
  };
      ...
      {success && (
        <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-4">
          <div className="flex items-center">
            ...
            <span className="text-green-800">Project updated successfully! Redirecting...</span>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => handleInputChange('budget_status', e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
