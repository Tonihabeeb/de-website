...
export default function MediaOrganize() {
  ...
  const getFileIcon = (mimeType: string) => {
    if (mimeType.startsWith('image/')) return <FileText className="w-6 h-6 text-blue-500" />;
    if (mimeType.startsWith('video/')) return <FileText className="w-6 h-6 text-purple-500" />;
    if (mimeType.startsWith('audio/')) return <FileText className="w-6 h-6 text-green-500" />;
    ...
    <div className="p-6">
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Media Organization</h1>
            <p className="text-gray-600 mt-1">Organize media with folders, tags, and bulk operations</p>
          </div>
            ...
            <div className="space-y-2">
              <button
                ...
                className={`w-full text-left px-3 py-2 rounded-lg transition-colors ${
                  filterFolder === 'all' ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50'
                }`}
              >
              ...
              {folders.map((folder) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg transition-colors flex items-center justify-between ${
                    filterFolder === folder.path ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50'
                  }`}
                >
                  ...
                  <span className="text-sm text-gray-500">{folder.item_count}</span>
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-2">
              {tags.map((tag) => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg transition-colors flex items-center justify-between ${
                    filterTags.includes(tag.name) ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-50'
                  }`}
                >
                  <span className="flex items-center">
                    <div 
                  ...
                  <span className="text-sm text-gray-500">{tag.item_count}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      {showBulkEdit && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Bulk Edit ({selectedItems.length} items)</h3>
