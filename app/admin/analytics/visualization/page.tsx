  ...
  const renderChart = (config: VisualizationConfig, index: number) => {
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className={`bg-gray-50 rounded-lg p-8 ${selectedVisualization === index ? 'min-h-96' : 'h-64'}`}>
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              {config.type === 'line' && <LineChart className="w-16 h-16 text-blue-500 mx-auto mb-4" />}
              {config.type === 'bar' && <BarChart3 className="w-16 h-16 text-green-500 mx-auto mb-4" />}
              {config.type === 'pie' && <PieChart className="w-16 h-16 text-purple-500 mx-auto mb-4" />}
              <h4 className="text-lg font-medium text-gray-900 mb-2">
                {config.dataSource.charAt(0).toUpperCase() + config.dataSource.slice(1)} Analytics
              </h4>
              <p className="text-gray-600">
                Interactive {config.type} chart showing {config.metrics.join(', ')} over {config.timeRange}
              </p>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="text-2xl font-bold text-gray-900">{visualizations.length}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {visualizations.map((visualization, index) => (
          <div key={index}>
            {renderChart(visualization, index)}
          </div>
        ))}
      </div>
      ...
      {visualizations.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-4">Create your first chart to start analyzing data</p>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { value: 'line', label: 'Line Chart', icon: LineChart, color: 'text-blue-500' },
                { value: 'bar', label: 'Bar Chart', icon: BarChart3, color: 'text-green-500' },
                { value: 'pie', label: 'Pie Chart', icon: PieChart, color: 'text-purple-500' },
                { value: 'doughnut', label: 'Doughnut', icon: PieChart, color: 'text-orange-500' },
                { value: 'area', label: 'Area Chart', icon: TrendingUp, color: 'text-red-500' }
              ].map((option) => {
                const Icon = option.icon;
                return (
                  <button
                    key={option.value}
                    onClick={() => setChartConfig({ ...chartConfig, type: option.value as any })}
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { value: 'projects', label: 'Project Analytics', icon: TrendingUp },
                ...
                return (
                  <button
                    key={option.value}
                    onClick={() => setChartConfig({ ...chartConfig, dataSource: option.value as any })}
                    className={`p-4 border rounded-lg text-center transition-colors ${
          ...
          <div>
            ...
            <div className="grid grid-cols-5 gap-3">
              {[
                ...
                <button
                  key={option.value}
                  onClick={() => setChartConfig({ ...chartConfig, timeRange: option.value as any })}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setChartConfig({
                          ...chartConfig,
                          metrics: chartConfig.metrics.filter(m => m !== metric)
                        });
                      }
                    }}
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => setChartConfig({ ...chartConfig, animate: e.target.checked })}
                className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
            ...
            <div className="space-y-4">
              <label className="flex items-center">
                <input
                  ...
                  className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                />
                <span className="ml-2 text-sm text-gray-700">Enable live updates</span>
              </label>
              <div>
