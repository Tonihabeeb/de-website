...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // If scrolled past the hero section (full viewport height), switch to original style
      setIsOverHero(scrollY < windowHeight);
    };
    ...
    <nav className={`sticky top-0 z-50 transition-all duration-300 ${
      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
        <div className='flex justify-between items-center h-16'>
            ...
            {isAuthenticated ? (
              <div className='relative' ref={userDropdownRef}>
                <button
                  ...
                  className={`flex items-center space-x-3 p-2 rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${
                    isHomePage && isOverHero ? 'hover:bg-white/20' : 'hover:bg-gray-50'
                  }`}
                ...
                >
                  ...
                  <div className='hidden lg:block text-left'>
                    <div className={`text-sm font-medium ${
                      isHomePage && isOverHero ? 'text-white' : 'text-gray-900'
                    }`}>
                      {user?.name}
                    </div>
                    <div className={`text-xs ${
                      isHomePage && isOverHero ? 'text-gray-200' : 'text-gray-700'
                    }`}>
                      {user?.role
                  ...
                  <ChevronDown
                    className={`w-4 h-4 transition-transform duration-200 ${
                      isHomePage && isOverHero ? 'text-gray-200' : 'text-gray-600'
          ...
          <button
            type='button'
            className={`md:hidden p-3 rounded-lg transition-all duration-200 min-w-[48px] min-h-[48px] focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${
              isHomePage && isOverHero
