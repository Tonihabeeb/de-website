...
export default function EnhancedHeroSection() {
  ...
  useEffect(() => {
    ...
    const check = () => setIsMobileOrReduced(mqMobile.matches || mqReduced.matches);
    check();
    ...
    <section className="relative overflow-hidden bg-gradient-to-br from-blue-50 to-indigo-100 min-h-[70vh] flex items-center justify-center">
      ...
      {isMobileOrReduced ? (
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden flex items-center justify-center">
          <Image src="/hero-static.svg" alt="Kinetic Power Plant" width={384} height={384} priority className="w-96 h-96" />
        </div>
      ) : showAnimation ? (
        <KPPHeroAnimation />
      ) : null}
