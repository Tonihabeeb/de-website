  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      ...
      >
        ...
        <span className="font-semibold text-primary">Drag & drop files here or click to select</span>
        <input
          ref={inputRef}
          type="file"
          multiple
      ...
      <div className="mb-4">
        <label htmlFor="tag-select" className="block mb-1 font-medium text-primary">Select Tag:</label>
        <select
          id="tag-select"
          ...
          <ul className="divide-y divide-gray-200">
            {files.map((file, i) => (
              <li key={i} className="flex items-center justify-between py-2">
                <span>{file.name} <span className="text-xs text-gray-500">({(file.size / 1024).toFixed(1)} KB)</span></span>
                <button
      ...
      <div className="mt-8">
        <h3 className="font-semibold text-lg mb-2 text-primary">Uploaded Files</h3>
        {Object.keys(filesByTag).length === 0 && <div className="text-gray-500">No files uploaded yet.</div>}
