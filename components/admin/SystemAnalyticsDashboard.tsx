...
export default function SystemAnalyticsDashboard({ className = '' }: { className?: string }) {
  ...
  const mockAnalyticsData: AnalyticsData = {
    ...
    topPages: [
      { path: '/', title: 'Homepage', views: 3450, uniqueViews: 2890, avgTimeOnPage: 145 },
      { path: '/technology', title: 'Technology', views: 2340, uniqueViews: 1890, avgTimeOnPage: 234 },
      { path: '/projects', title: 'Projects', views: 1890, uniqueViews: 1450, avgTimeOnPage: 189 },
      { path: '/about', title: 'About Us', views: 1230, uniqueViews: 980, avgTimeOnPage: 98 },
      { path: '/contact', title: 'Contact', views: 890, uniqueViews: 720, avgTimeOnPage: 67 },
    ],
    userActivity: [
    ...
    return (
      <div className={`bg-white rounded-lg shadow-sm border ${className}`}>
        <div className="p-6 text-center">
          ...
          <p className="text-gray-600 mb-4">Failed to connect to analytics service.</p>
    ...
    <div className={`bg-white rounded-lg shadow-sm border ${className}`}>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-green-600 mt-1">
                  +{getGrowthRate(analyticsData.overview.totalUsers, 1200).toFixed(1)}% from last period
                </p>
              </div>
          ...
          <div className="bg-white rounded-lg p-4 border">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-green-600 mt-1">
                  +{getGrowthRate(analyticsData.overview.activeUsers, 310).toFixed(1)}% from last period
                </p>
              </div>
          ...
          <div className="bg-white rounded-lg p-4 border">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{formatNumber(analyticsData.overview.pageViews)}</p>
                <p className="text-sm text-green-600 mt-1">
                  +{getGrowthRate(analyticsData.overview.pageViews, 14200).toFixed(1)}% from last period
                </p>
              </div>
          ...
          <div className="bg-white rounded-lg p-4 border">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{formatPercentage(analyticsData.overview.bounceRate)}</p>
                <p className="text-sm text-red-600 mt-1">
                  +{getGrowthRate(analyticsData.overview.bounceRate, 22.1).toFixed(1)}% from last period
                </p>
              </div>
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="h-64 flex items-center justify-center bg-white rounded border">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-600">Chart visualization would be here</p>
                <p className="text-xs text-gray-500 mt-1">Using Chart.js or similar library</p>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
              <div>
                <p className="text-gray-600">New Users</p>
                <p className="font-medium">{formatNumber(analyticsData.userGrowth.reduce((sum, day) => sum + day.newUsers, 0))}</p>
              </div>
              <div>
                <p className="text-gray-600">Avg Daily Active</p>
                <p className="font-medium">{formatNumber(Math.round(analyticsData.userGrowth.reduce((sum, day) => sum + day.activeUsers, 0) / analyticsData.userGrowth.length))}</p>
              </div>
            </div>
          </div>
          ...
          {/* Page Views Chart */}
          <div className="bg-gray-50 rounded-lg p-4">
            <h4 className="text-lg font-medium text-gray-900 mb-4">Page Views</h4>
            <div className="h-64 flex items-center justify-center bg-white rounded border">
              <div className="text-center">
            ...
            <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium">{formatNumber(analyticsData.pageViews.reduce((sum, day) => sum + day.views, 0))}</p>
              </div>
              <div>
                <p className="text-gray-600">Unique Views</p>
                <p className="font-medium">{formatNumber(analyticsData.pageViews.reduce((sum, day) => sum + day.uniqueViews, 0))}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Device Statistics */}
          <div className="bg-gray-50 rounded-lg p-4">
            <h4 className="text-lg font-medium text-gray-900 mb-4">Device Usage</h4>
              ...
              {analyticsData.deviceStats.map((device, index) => (
                <div key={index} className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium text-gray-900">{formatNumber(device.users)}</p>
            ...
            <div className="space-y-3">
              {analyticsData.geographicData.slice(0, 5).map((geo, index) => (
                <div key={index} className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium text-gray-900">{formatNumber(geo.users)}</p>
                    <p className="text-xs text-gray-500">{formatPercentage(geo.percentage)}</p>
                  </div>
                </div>
              ))}
            </div>
