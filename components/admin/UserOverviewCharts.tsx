...
const UserOverviewCharts: React.FC<UserOverviewChartsProps> = ({ className = '' }) => {
  const [userStats, setUserStats] = useState<UserStats | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedChart, setSelectedChart] = useState<'overview' | 'growth' | 'roles' | 'activity'>('overview');
  ...
  const fetchUserStats = async () => {
    try {
      ...
      const mockStats: UserStats = {
        ...
        top_users: [
          { user_id: '1', username: 'admin@example.com', activity_count: 45, last_activity: new Date().toISOString() },
          { user_id: '2', username: 'editor@example.com', activity_count: 32, last_activity: new Date(Date.now() - 3600000).toISOString() },
          { user_id: '3', username: 'author@example.com', activity_count: 28, last_activity: new Date(Date.now() - 7200000).toISOString() },
        ],
      };
  ...
  const roleDistributionData = userStats ? Object.entries(userStats.role_distribution).map(([role, count]) => ({
  ...
  return (
    <div className={`bg-white rounded-lg shadow-sm border ${className}`}>
      <div className="p-6">
            ...
            <div className="bg-orange-50 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-orange-900">{userStats.new_users_this_month}</p>
                </div>
            ...
            <div className="bg-purple-50 p-4 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-900">{userStats.user_growth_rate}%</p>
                </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={userStats.user_activity_timeline}>
                ...
                <Tooltip 
                  ...
                  formatter={(value, name) => [value, name === 'active_users' ? 'Active Users' : 'New Users']}
                />
                <Area 
                  type="monotone" 
        ...
        {selectedChart === 'overview' && (
          <div className="mt-6">
            <h4 className="text-sm font-medium text-gray-900 mb-3">Top Active Users</h4>
