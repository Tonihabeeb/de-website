...
import { Upload, Image, File, Folder, Search, Filter, Grid, List, Download, Trash2, Edit, Eye, Plus } from 'lucide-react';
...
const MediaManager: React.FC<MediaManagerProps> = ({
  ...
  const filteredMedia = media.filter(item => {
    ...
    const matchesType = selectedType === 'all' || 
                       ...
                       (selectedType === 'documents' && item.mime_type.startsWith('application/'));
  ...
  const getFileIcon = (mimeType: string) => {
    ...
    if (mimeType.startsWith('video/')) return <File className="w-6 h-6 text-purple-500" />;
              ...
              >
                {viewMode === 'grid' ? (
                  <div className="relative group">
                    ...
                    <div className="p-2">
                      <p className="text-xs font-medium text-gray-900 truncate">{item.original_name}</p>
