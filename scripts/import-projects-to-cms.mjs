...
async function importProjects() {
  ...
  try {
    ...
    if (!tableExists) {
      console.log('❌ Projects table does not exist. Please run database migrations first.');
      return;
    }
    ...
    for (const projectData of projectsData) {
      ...
      const existingProject = db.prepare('SELECT id FROM projects WHERE slug = ?').get(projectData.slug);
    ...
    console.log(`📁 Total projects in database: ${importedCount + skippedCount}`);

    // Display imported projects
    if (importedCount > 0) {
      console.log('\n📋 Imported Projects:');
      const importedProjects = db.prepare('SELECT name, slug, status, capacity_mw FROM projects ORDER BY created_at DESC LIMIT ?').all(importedCount);
    ...
    console.log('\n📝 Next steps:');
    console.log('1. Access the admin panel at /admin/projects to manage projects');
