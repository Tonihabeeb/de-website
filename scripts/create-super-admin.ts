...
async function createSuperAdmin() {
  try {
    ...
    const existingSuperAdmin = await UserModel.findAll({ role: UserRole.SUPER_ADMIN });
    ...
    console.error('❌ Error creating super admin:', error instanceof Error ? error.message : 'Unknown error');
    process.exit(1);
  } finally {
    rl.close();
  }
